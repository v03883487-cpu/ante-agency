    ...
    <section className="border-y border-black/10 bg-white py-14">
      <div className="mx-auto max-w-5xl px-6 sm:px-12">
        <div className="mb-8 flex flex-wrap items-center justify-between gap-3">
          ...
          {withLinkToPage && (
            <Link href="/brands" className="text-xs font-medium text-[#0A0A0A] hover:opacity-60">
              Все бренды →
            </Link>
          )}
        </div>
        ...
        <div className="flex w-max animate-marquee gap-3 [animation-play-state:running] hover:[animation-play-state:paused]">
          {track.map((b, i) => (
            <a
              ...
              className="group shrink-0 rounded-full border border-black/10 px-5 py-2.5 text-sm text-zinc-700 transition-colors hover:border-black/40"
            >
